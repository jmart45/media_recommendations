...
def db_update_media_item(
    ...
    """Update any combination of genre, rating, and notes for an existing item."""
    updates: dict = {}
    ...
    with get_conn() as conn:
        ...
        if type_id is None:
            return err(f"Media type '{media_type}' not found.")
        cur = conn.execute(
            f"UPDATE media_items SET {set_clause} WHERE title = ? COLLATE NOCASE AND media_type_id = ?",
            list(updates.values()) + [title, type_id],
        )
        if cur.rowcount == 0:
            return err(f"'{title}' not found in {media_type}.")
        return ok(f"Updated '{title}'.")


def db_update_media_genre(title: str, media_type: str, genre: Optional[str]) -> dict:
    with get_conn() as conn:
        type_id = _resolve_type_id(conn, media_type)
        if type_id is None:
            return err(f"Media type '{media_type}' not found.")
        cur = conn.execute(
            "UPDATE media_items SET genre = ? WHERE title = ? COLLATE NOCASE AND media_type_id = ?",
            (genre, title, type_id)
        )
        if cur.rowcount == 0:
            return err(f"'{title}' not found in {media_type}.")
        return ok(f"Updated genre for '{title}'.")
